...
/**
 * GET /api/admin/servers/[id]/databases
 * Proxy zu Agent /databases - Liste aller Datenbanken
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    try {
      const agentUrl = `http://${server.host}:${server.agent_port}/databases`;
      ...
      if (!response.ok) {
        return NextResponse.json({ error: 'Agent-Fehler', databases: [] }, { status: 502 });
      }
      ...
      return NextResponse.json({ error: 'Server nicht erreichbar', databases: [] }, { status: 503 });
    }
